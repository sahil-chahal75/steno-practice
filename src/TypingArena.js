...
const TypingArena = ({ doc, setTypingResultData }) => {
  ...
  useEffect(() => {
    const handleBeforeUnload = (e) => {
      if (isActive && !isFinished) {
        e.preventDefault();
        e.returnValue = "Examination in progress. Data will be lost if you leave.";
      }
    };
  ...
  useEffect(() => {
    ...
    if (isActive && timeLeft > 0) {
      ...
      calculateAndSubmit(true); // Auto-submit on time out
    }
  ...
  const calculateAndSubmit = (isTimeOut = false) => {
    setIsFinished(true);
    ...
    // 🛠️ PATCH: Calculating Actual Time Spent
    const timeSpentInSeconds = (selectedTime * 60) - timeLeft;
    const timeSpentInMinutes = isTimeOut ? selectedTime : Math.max(0.1, timeSpentInSeconds / 60);
    ...
    typedWords.forEach((word, i) => {
      // Infinite Looping Comparison Logic
      const originalWord = originalWords[i % originalWords.length];
      if (word !== originalWord) mistakes++;
    });

    // Speed calculation using actual time spent
    const netWpm = Math.max(0, (totalWords - mistakes) / timeSpentInMinutes);
    ...
    setTypingResultData({
      ...
      timeTaken: parseFloat(timeSpentInMinutes.toFixed(2))
    });
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 animate-in fade-in duration-700 font-sans">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-[3rem] shadow-2xl mb-6 border-b-8 border-green-600 flex flex-col md:flex-row justify-between items-center gap-6">
        ...
        <div className="flex items-center gap-4 bg-slate-100 dark:bg-slate-700 p-2 rounded-2xl">
          {[1, 3, 5, 10].map(m => (
            <button 
              ...
              disabled={isActive}
              className={`px-4 py-2 rounded-xl text-[10px] font-black transition-all ${selectedTime === m ? 'bg-green-600 text-white shadow-lg' : 'text-slate-500'}`}
            >
              {m} MIN
            </button>
          ))}
        ...
        <div className="text-center md:text-right min-w-[120px]">
          ...
          <p className={`text-4xl font-black font-mono ${timeLeft < 60 ? 'text-red-600 animate-pulse' : 'text-slate-800 dark:text-white'}`}>
      ...
      {/* MATTER DISPLAY */}
      <div className="bg-slate-50 dark:bg-slate-900/50 p-10 rounded-[3rem] border-2 border-slate-200 dark:border-slate-800 mb-6 shadow-inner overflow-y-auto max-h-[350px]">
        ...
        <div className="mt-8 pt-4 border-t border-slate-200 dark:border-slate-700">
          <p className="text-[10px] font-black text-blue-600 uppercase italic">
            Note: If you finish the text, restart from the first word. Accuracy is key.
          </p>
        </div>
      </div>
      ...
      <div className="relative">
        <textarea
          ...
          placeholder="Select time and click 'Start'..."
          value={input}
      ...
      <div className="mt-8 flex justify-between items-center">
        <button 
          ...
          className={`px-6 py-3 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all ${backspaceDisabled ? 'bg-red-600 text-white shadow-lg' : 'bg-slate-200 text-slate-500'}`}
        >
          Backspace: {backspaceDisabled ? 'LOCKED' : 'ENABLED'}
        </button>

        <button onClick={() => calculateAndSubmit(false)} className="bg-slate-900 dark:bg-white dark:text-slate-900 text-white px-12 py-5 rounded-3xl font-black uppercase tracking-widest shadow-xl active:scale-95 transition-all">
          Submit Assessment
        </button>
      </div>
    </div>
