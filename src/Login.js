  ...
  const handleAuth = async (e) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        alert("Account Created! You can now browse magazines.");
      } else if (mode === 'login') {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 flex flex-col items-center justify-center p-6 transition-colors">
      ...
      <div className="bg-white dark:bg-slate-800 p-8 rounded-[2.5rem] shadow-2xl w-full max-w-md border-t-8 border-blue-600">
        <div className="text-center mb-8">
          ...
          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mt-1">Practice to Perfection</p>
        </div>

        <form onSubmit={handleAuth} className="space-y-4">
          <h2 className="text-xl font-black dark:text-white uppercase text-center mb-4 italic">
            {mode === 'signup' ? 'Create Profile' : mode === 'reset' ? 'Reset Password' : 'Secure Login'}
          </h2>
          ...
          <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-xl font-black uppercase tracking-widest shadow-lg active:scale-95 transition-all">
            {mode === 'signup' ? 'Sign Up' : mode === 'reset' ? 'Send Link' : 'Login'}
          </button>
        </form>
        ...
        <div className="mt-8 text-center flex flex-col gap-2">
          {mode === 'login' ? (
            <>
              <button onClick={() => setMode('signup')} className="text-blue-600 text-xs font-black uppercase tracking-widest underline">New User? Create Account</button>
              <button onClick={() => setMode('reset')} className="text-slate-400 text-[10px] font-bold uppercase">Forgot Password?</button>
            </>
          ) : (
            <button onClick={() => setMode('login')} className="text-blue-600 text-xs font-black uppercase tracking-widest underline">Already have an account? Login</button>
          )}
        </div>
      </div>
    </div>
